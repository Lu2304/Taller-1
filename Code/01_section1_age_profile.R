...
# ----------------------------- #
if (!dir.exists("Bases")) dir.create("Bases")
if (!dir.exists("Outputs")) dir.create("Outputs")
if (!dir.exists(file.path("Outputs", "tables"))) dir.create(file.path("Outputs", "tables"), recursive = TRUE)
if (!dir.exists(file.path("Outputs", "figures"))) dir.create(file.path("Outputs", "figures"), recursive = TRUE)

path_tables <- file.path("Outputs", "tables")
path_figs   <- file.path("Outputs", "figures")

# ----------------------------- #
# 2) WEB SCRAPING (10 CHUNKS)
# ----------------------------- #
urls <- paste0(
...
# Validar que columnas coinciden
nombres_columnas <- lapply(lista_tablas, names)
ref <- nombres_columnas[[1]]
stopifnot(all(sapply(nombres_columnas, function(x) identical(x, ref))))

# Unir bases
base_completa <- bind_rows(lista_tablas)

# (Opcional) guardar raw unido
saveRDS(base_completa, file = file.path("Bases", "base_completa.rds"))

# ----------------------------- #
# 3) LIMPIEZA Y MUESTRA DE ANÁLISIS
# ----------------------------- #
# Restricción del taller: 18+ y ocupados
base_samp <- base_completa |>
  filter(age >= 18, ocu == 1)

# Mantener solo ingresos válidos para log (no NA y > 0)
...
# Guardar base limpia
saveRDS(base_clean, file = file.path("Bases", "base_clean.rds"))
